const Choices = {
        allRoles: `SELECT r.id, r.title AS job_title, r.salary, d.name AS department_name
                    FROM role r
        ...
        getDepartments: `SELECT id, name AS department_name FROM department`,
        getRoles: `SELECT id, title FROM role`,
        getManagers: `SELECT id, CONCAT(first_name, ' ', last_name) AS name FROM employee`,
